...
public function indexJson(\Illuminate\Http\Request $request)
{
    ...
    private function isInactiveProgram(array $program): bool
    {
        ...
        $statusTexts = is_array($program['status_texts'] ?? null) ? $program['status_texts'] : [];

        $statuses = array_filter([
            mb_strtolower(trim((string) ($program['program_status'] ?? ""))),
            mb_strtolower(trim((string) ($program['status_label'] ?? ""))),
            ...array_map(static fn ($value): string => mb_strtolower(trim((string) $value)), $statusTexts),
        ]);

        foreach ($statuses as $status) {
            if ($status === "0" || $status === "inactive" || str_contains($status, "discontinu") || str_contains($status, "closed") || str_contains($status, "not offered") || str_contains($status, "phase out")) {
                return true;
            }
        }
    ...
    {
        $schoolName = trim((string) ($app->intended_school_name ?? $app->other_school ?? ''));
        $courseName = trim((string) ($app->course_name ?? $app->gad_stufaps_course ?? ''));
